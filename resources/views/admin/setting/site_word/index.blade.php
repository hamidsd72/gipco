        ...
        <div class="card-header  border-0">
          <h4 class="w-100">
            ...
            @if(auth()->user()->hasRole('developer'))
              @can('site_word_create')
                <a href="{{route('admin.site-word.create')}}" class="btn btn-primary float-left">افزودن</a>
              @endcan
            @endif
          </h4>
        </div>
          ...
          <div class="table-responsive">
            <table class="table  table-vcenter text-nowrap table-bordered border-bottom" id="tbl_1">
              ...
              <tr>
                ...
                <th class="border-bottom-0">مکان اصلی</th>
                <th class="border-bottom-0">مکان داخلی</th>
                <th class="border-bottom-0">متن</th>
                @canany(['site_word_edit','site_word_delete'])
                ...
                <tr>
                  ...
                  @canany(['site_word_edit','site_word_delete'])
                    <td>
                      <div class="d-flex">
                        @can('site_word_edit')
                          <a href="{{route('admin.site-word.edit',$item->id)}}"
                             class="action-btns1">
                        ...
                        @endcan
                        @if(auth()->user()->hasRole('developer'))
                          @can('site_word_delete')
                            {!! Form::open(['method' => 'DELETE', 'route' => ['admin.site-word.destroy', $item->id] ]) !!}
