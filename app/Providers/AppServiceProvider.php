...
class AppServiceProvider extends ServiceProvider
{
    ...
    public function boot(Request $request)
    {
        ...
        view()->composer('layouts.includes_front.mobile_menu', function ($view) {
            $links=Navbar::whereIn('type', ['top', 'both'])->where('parent_id', null)->orderBy('sort')->get(['id','title','link']);

            $view
                ->with('links', $links);
        });
        view()->composer('layouts.includes_front.header', function ($view) {
            $links=Navbar::whereIn('type', ['top', 'both'])->where('parent_id', null)->orderBy('sort')->get(['id','title','link']);

            $view
                ->with('links', $links);
        });
        view()->composer('layouts.includes_front.footer', function ($view) {
            $contact=Contact::first();
            $links=Navbar::whereIn('type', ['footer', 'both'])->where('parent_id', null)->orderBy('sort')->get(['id','title','link']);
